...
void displayname(){
    ...
    lcd_writecommand(1);
	char date[30]; //array to store my birthday
	char display_count[30]; //array used to display count.
    lcd_moveto(0,0);
	lcd_stringout("Jui Po Hung");
    lcd_moveto(1,2);
   	snprintf(date, 30, "%2d/%2d/%4d", 04, 18, 1999);
    lcd_stringout(date);
    _delay_ms(1000);
    lcd_writecommand(1);
}
...
/*
  ...
  Copy the upper 4-bits of the lcdbits argument to PORTD[7:4]
*/
